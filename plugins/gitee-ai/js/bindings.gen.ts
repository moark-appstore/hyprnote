...
async sendVerificationCode(email: string) : Promise<null> {
    return await TAURI_INVOKE("plugin:gitee-ai|send_verification_code", { email });
},
...
async pay(planIdent: string, quantity: number, payType: string) : Promise<GiteeAiPayDetail> {
    return await TAURI_INVOKE("plugin:gitee-ai|pay", { planIdent, quantity, payType });
},
